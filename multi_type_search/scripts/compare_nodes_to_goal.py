...
def __process_wrapper__(kwargs):
    ...
    return compare_nodes_to_goal(**kwargs)


def compare_nodes_to_goal_from_file(
        input_file: Path,
        score_name: str,
    ...
    """
    Loads a file that contains a List of Graph objects with nodes to compare with the goal of the graph.  Check
    compare_nodes_to_goal for more information on what the comparison is (this function wraps it).
    ...
    :param comparison_metric: The comparison metric that will be used to compare the node and the goal (Can also be
        the json configuration of a comparison metric)
    ...
    for idx, (torch_device, graphs) in enumerate(zip(torch_devices, device_graphs)):
        ...
        compare_nodes_to_goal_calls.append(call_args)

    if len(torch_devices) == 1:
        results = [compare_nodes_to_goal(**compare_nodes_to_goal_calls[0])]
    else:
        with Pool(len(torch_devices)) as p:
            results = p.map(__process_wrapper__, compare_nodes_to_goal_calls)
...
def compare_nodes_to_goal(
    ...
    """
    Given a list of graphs, compare each node with each goal and save it's score under each nodes node.score attribute.
    ...
    :param __job_idx__: If this function was called via compare_nodes_to_goal or is being ran in a multiprocessed
    ...
    for graph in tqdm(
            graphs,
            desc='Comparing Graph Nodes with Goal',
            total=len(graphs),
            leave=False,
        ...
        for allowed_node_type in allowed_graphkeytypes:
            ...
            if isinstance(nodes[0], Node):
                ...
                targets = [graph.goal.normalized_value if use_normalized_values else graph.goal.value] * len(nodes)
                scores = comparison_metric.score(group, targets)

                for node, score in zip(nodes, scores):
                    node.scores[score_name] = score
                ...
                for hypernode in nodes:
                    ...
                    targets = [graph.goal.normalized_value if use_normalized_values else graph.goal.value] * len(group)
                    scores = comparison_metric.score(group, targets)
