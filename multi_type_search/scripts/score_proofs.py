...
from gpt3.score_prompt import score as gpt3_score
from multi_type_search.utils.paths import SEARCH_DATA_FOLDER
from multi_type_search.search.tree.tree import Tree
from multi_type_search.search.tree.step_generation import StepGeneration
from multi_type_search.search.evaluation.metrics.entailement_metric import EntailmentEvaluation
from multi_type_search.search.step_type import StepModel, ForwardStepModel
from multi_type_search.search.validator import GPT3Validator
...
def __score_proofs__(
    ...
    for tree in tqdm(
        ...
        for proof in tree:
            ...
            if 'gpt3' in score_methods:

                for idx, intermediate in enumerate(proof.intermediates):

                    # Try to use the GPT3 cached score so we can avoid extra calls.
                    if 'gpt3_output_score' in intermediate.scores:
                        proof_scores['gpt3'] += intermediate.scores.get('gpt3_output_score')
                    else:
                        _score = gpt3_score(intermediate.output, '')
                        intermediate.scores['gpt3_output_score'] = _score
                        proof_scores['gpt3'] += _score

                proof_scores['gpt3'] /= len(proof.intermediates)
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--score_methods', '-st', type=str, nargs="+", required=True,
                           choices=["forward_agreement", "gpt3"],
